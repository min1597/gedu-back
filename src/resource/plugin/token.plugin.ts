...
export default {
        ...
        > => {
            try {
                ...
                return {
                    ...
                    signout: async () => {
                        try {
                            if(_tokens[0].user_id == null) return { success: false, error: new Error('Already signed out.') }
    ...
    Authorization: {
        ...
        > => {
            try {
                const _result = await axios.post(`${ process.env.LUNA_ACCOUNTS_API_ENDPOINT }/oauth2/token`, { grant_type: 'client_credentials', client_id: process.env.LUNA_ACCOUNTS_CLIENT_ID, client_secret: process.env.LUNA_ACCOUNTS_CLIENT_SECRET })
                if(_result.status !== 200) return { success: false, error: new Error('Failed to issue App token.') }

                return { success: true, token: { tokenType: _result.data.token_type, token: _result.data.access_token as string & { __brand: 'TOKEN' } } }
        ...
        > => {
            try {
                ...
                const _result = await axios.post(
                    `${ process.env.LUNA_ACCOUNTS_API_ENDPOINT }/oauth2/token`,
                    {
                        ...
                        redirect_uri: _data.type == 'code' ? new URL(_data.redirectUri).origin : undefined,
                        code_verifier: _data.type == 'code' ? _data.codeVerifier : undefined,
                        refresh_token: _data.type == 'token' ? _data.token : undefined
                    },
                    { headers: { 'X-APPTOKEN': `${ _appToken.token.tokenType } ${ _appToken.token.token }` } }
                )
                ...
                return {
                    success: true,
                    token: {
                        tokenType: _result.data.token_type,
                        accessToken: _result.data.access_token as string & { __brand: 'TOKEN' },
                        refreshToken: _result.data.refresh_token as string & { __brand: 'TOKEN' },
                    },
                    scope: _result.data.scope.split(' ')
                }
            } catch(_error) { return _error instanceof Error ? { success: false, error: new Error('An unknown error has occured', { cause: _error }) } : (typeof _error == 'string' ? { success: false, error: new Error(_error) } : { success: false, error: new Error('An unknown error has occured.') }) }
        }
    },
    ...
    > => {
        try {
            ...
            await getDatabaseClient().manager.getRepository(Token).update({ uuid: _tokens[0].uuid }, { is_active: false })
